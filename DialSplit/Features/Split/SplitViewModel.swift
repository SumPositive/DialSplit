...
final class SplitViewModel {
    ...
    @ObservationIgnored private var _persons0:      Int = 1
    @ObservationIgnored private var _persons1:      Int = 0
    @ObservationIgnored private var _persons2:      Int = 0
    ...
    var dialUnitIndex: Int {
        ...
        set {
            ...
            // 単位変更時: 既存金額を新単位に丸める（バッキングストレージを直接更新して通知）
            let u = dialUnit
    ...
    /// 大富豪1人あたり（自動計算・¥1単位切上）
    /// WariKan 互換: 富豪/平民への分配後の残余を大富豪で均等割り、端数は大富豪が+¥1で被る
    var split0: Int {
        let sum0 = totalRaw - persons1 * split1 - persons2 * split2
    ...
    var totalPersons: Int { persons0 + persons1 + persons2 }
